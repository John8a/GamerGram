...
app.get("/dashboard", authenticateMe, async (req, res) => {
...
app.get("/dashboard/abo", exportExcelData, authenticateMe, async (req, res) => {
    const emails = await Abonnements.find();
    res.render("dashboard/abonnements", { emails });
});

app.delete("/abo/:id", authenticateMe, async (req, res) => {
    const { id } =  req.params; 
    console.log(id);
...
app.delete("/poll/:id", authenticateMe, async (req, res) => {
    const { id } =  req.params; 
