...
const worksheet = workbook.addWorksheet("Abonennten");


try {
    mongoose.connect('mongodb://localhost/gamergram', { useNewUrlParser: true });
} catch (error) {
    handleError(error);
}
...
app.use(async (req, res) => {
    const abos = await Abonnements(req.body);
    worksheet.colums = [{header: 'Datum'}, {header: 'E-Mail'}];
    abos.forEach(email => {
        worksheet.addRow({dob: new Date(email.date), email: email.email});
    });

    await workbook.xlsx.writeFile('abonements');
});
